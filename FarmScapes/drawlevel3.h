...
// ============================================================
// LOGIC UPDATES (Called inside iSetTimer)
// ============================================================
...
void updateLevel3Logic()
{
	...
	// Countdown for the "Line Has Been Cast" pop-up banner
	if (castPopupTimer > 0)
	{
		castPopupTimer--;
	}
	...
	if (fishingState == FISH_STATE_WAITING)
	{
		...
		else
		{
			...
			if (chance < 30) // 30% chance nothing bites
			{
				fishingState = FISH_STATE_NOTHING;
				resultDisplayTimer = 40; // ~2 seconds display
			}
			else // 70% chance fish hooks
			{
				fishingState = FISH_STATE_HOOKED;
				hookTimer = 45; // ~2.2 seconds reaction window to click
			}
		}
	}

	// 2. Fish is hooked on line (player must click before hookTimer runs out)
	if (fishingState == FISH_STATE_HOOKED)
	{
		...
		{
			// Player reacted too slowly!
			fishingState = FISH_STATE_NOTHING;
			resultDisplayTimer = 40;
		}
	}

	// 3. Reset display timers back to idle state
...
void drawLevel3()
{
	...
	if (isFishMarketOpen)
	{
		...
		// Close Button (Draw at X: 360, Y: 100, W: 80, H: 30)
		iSetColor(200, 50, 50);
...
void handleLevel3Keyboard(unsigned char key)
{
	...
	if (key == 27)
	{
		gameState = STATE_TOWN; // ESC goes back to town
	}
}
...
void handleLevel3MouseClick(int mx, int my)
{
	...
	if (isFishMarketOpen)
	{
		...
		// Sell Buttons for Fish Types (X: 520 to 590)
		for (int i = 0; i < NUM_FISH_TYPES; i++)
		{
			int itemY = 365 - (i * 40);
		...
		return; // Block click from reaching game field
	}

	// 2. IGNORE TOP HEADER BAR CLICKS
	if (my >= 540)
	{
		return;
	}

	// 3. FISHING GAMEPLAY INPUT
	if (fishingState == FISH_STATE_IDLE)
	{
		fishingState = FISH_STATE_WAITING;
		fishingWaitTimer = 30 + rand() % 30; // Random wait before fish bites
		castPopupTimer = 15;                  // Show "Line Cast" pop-up briefly
	}
	else if (fishingState == FISH_STATE_HOOKED)
	{
		// Success! Reel in the fish
	...
	else if (fishingState == FISH_STATE_CAUGHT || fishingState == FISH_STATE_NOTHING)
	{
		// Click to dismiss result instantly and reset rod
		fishingState = FISH_STATE_IDLE;
		resultDisplayTimer = 0;
	}
}
